...
describe('税・控除（配偶者課税・退職金課税・NISA・iDeCo・住宅ローン控除）のテスト', () => {
  ...
  describe('12. NISA / 特定口座', () => {
    ...
    it('赤字で資産が尽きても投資資産・運用益がマイナスにならないこと', () => {
      // 30歳に1000万を投資へ。31歳以降は給与ゼロで毎年2000万の赤字。
      // 現金→投資資産を取り崩しても、投資資産は0で止まり負債は現金マイナスに残る。
      const params = {
        ...DEFAULT_PARAMS,
        startAge: 30,
        endAge: 33,
        ...
        idecoAnnual: 0,
        mortgageDeduction: false,
        basicLivingCost: 2000, // 毎年2000万の生活費=大赤字
        rentBeforeBuy: 0,
        spouseLivingCost: 0,
        childLivingCost: 0,
        tempExtraLivingCost: null,
      ...
      const r = simulate(params);
      r.forEach((row) => {
        expect(row.cumulativeInvestment).toBeGreaterThanOrEqual(0);
        expect(row.investmentYieldIncome).toBeGreaterThanOrEqual(0);
      });
    });
  });

  describe('12b. 収入の中断・辞める年齢', () => {
    it('本人の一時中断(育休)期間中は給与・手取りが0になること', () => {
      const params = { ...SAMPLE_PARAMS, selfIncomePause: { startAge: 32, endAge: 33 } };
      const r = simulate(params);
      expect(r.find((x) => x.age === 31)!.salary).toBeGreaterThan(0);
      expect(r.find((x) => x.age === 32)!.salary).toBe(0);
      expect(r.find((x) => x.age === 32)!.takeHome).toBe(0);
      expect(r.find((x) => x.age === 33)!.salary).toBe(0);
      expect(r.find((x) => x.age === 34)!.salary).toBeGreaterThan(0); // 復帰
    });

    it('配偶者が辞める年齢以降は配偶者収入が0になること', () => {
      const params = { ...SAMPLE_PARAMS, spouseIncomeStartAge: 30, spouseIncomeEndAge: 40, spouseIncomeAmount: 300 };
      const r = simulate(params);
      expect(r.find((x) => x.age === 40)!.spouseIncome).toBeGreaterThan(0);
      expect(r.find((x) => x.age === 41)!.spouseIncome).toBe(0);
    });

    it('配偶者の一時中断(育休)期間中は配偶者収入が0になること', () => {
      const params = { ...SAMPLE_PARAMS, spouseIncomeStartAge: 30, spouseIncomeAmount: 300, spouseIncomePause: { startAge: 33, endAge: 34 } };
      const r = simulate(params);
      expect(r.find((x) => x.age === 32)!.spouseIncome).toBeGreaterThan(0);
      expect(r.find((x) => x.age === 33)!.spouseIncome).toBe(0);
      expect(r.find((x) => x.age === 34)!.spouseIncome).toBe(0);
      expect(r.find((x) => x.age === 35)!.spouseIncome).toBeGreaterThan(0); // 復帰
    });

    it('新フィールドが未定義(旧localStorage相当)でも落ちないこと', () => {
      // 旧バージョンで保存されたデータには新項目が無い(undefined)
      const legacy = { ...SAMPLE_PARAMS } as Record<string, unknown>;
      delete legacy.selfIncomePause;
      delete legacy.spouseIncomeEndAge;
      delete legacy.spouseIncomePause;
      expect(() => simulate(legacy as unknown as typeof SAMPLE_PARAMS)).not.toThrow();
    });
  });
